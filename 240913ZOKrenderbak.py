import pip
   ...
   ## para instalar la librería cryptography
...
if __name__ == '__main__':
    #app.run(debug=True)
    app.run(debug=True, https://ibercaja.onrender.com, port=5001)

import requests

response = requests.get('https://ibercaja.onrender.com')##http://localhost:5001')
print(response.text)


variable = datetime.now() ##"¡Hola, mundo!"
print("linea 205 ", variable)
...
def internal():
    ...
    return redirect(url_for('https://127.0.0.1:5004'))
...
def before_request():
    if not request.is_secure:
        ...
        ##return redirect("https://ibercaja.onrender.com/")
        return redirect(url, code=301)

@app.route('/')
def index():
    
    #variable = "¡Hola, home X!"
    return render_template('index.html',variable = variable)    
